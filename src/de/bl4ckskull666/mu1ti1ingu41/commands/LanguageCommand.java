...
public class LanguageCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender s, Command c, String l, String[] a) {
        if(a.length > 0) {
            if(a[0].equalsIgnoreCase("reload")) {
                ...
                if(!s.hasPermission("mu1ti1ing41.admin")) {
                    s.sendMessage(Language.getMu1ti1ingu41Text(uuid, "command.reload.no-permission", "You dont have permission to use this command."));
                    return true;
                }
                
                Language.loadLanguage();
                s.sendMessage(Language.getMu1ti1ingu41Text(uuid, "command.reload.successful", "Language files has been reloaded."));
                return true;
            }
        }
        ...
        Player p = (Player)s;
        if(a.length < 1) {
            p.sendMessage(Language.getMu1ti1ingu41Text(p.getUniqueId(), "command.need-language", "Please select one of the following language :"));
            String ava = "";
        ...
        if(!Mu1ti1ingu41.getPlugin().getConfig().isString("short-language." + a[0].toLowerCase())) {
            p.sendMessage(Language.getMu1ti1ingu41Text(p.getUniqueId(), "command.unknown-language", "Can't find the wished language."));
            return true;
        }
        
        UUIDLanguages._players.put(p.getUniqueId(), Mu1ti1ingu41.getPlugin().getConfig().getString("short-language." + a[0].toLowerCase()));
        p.sendMessage(Language.getMu1ti1ingu41Text(p.getUniqueId(), "command.changed-language", "The Wished Language has been changed."));
        return true;
    }
}